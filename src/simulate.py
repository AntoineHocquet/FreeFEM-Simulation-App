...
def run_simulation(pde=None):
    """Run a PDE from the catalogue. ``pde`` overrides params['pde'].
    ...
    if pde is None:
        # Legacy path: original heat_disk template at edp/ root.
    ...
    merged.update({k: v for k, v in params.items() if k != "pde"})
